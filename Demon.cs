...
{
    public Stats demonStats;
    public int hp, mp;
    public int experienceGifted = 10;
    ...
    {
        this.demonStats = batStats;
        hp = Formulas.GetMaxHp(demonStats);
        mp = Formulas.GetMaxMp(demonStats);
    }

    private void Awake()
    {
        Initialize(demonStats);
    }
    ...
    void Attack()
    {
        ...
        Player.Instance.playerStats.hp -= Formulas.GetPhysicalDamage(demonStats, Player.Instance.playerStats, 1);
    ...
    private void OnTriggerEnter2D(Collider2D coll)
    {
        ...
        {
            hp -= Formulas.GetPhysicalDamage(Player.Instance.playerStats, demonStats, 1);
            damageReceived.text = "" + Formulas.GetPhysicalDamage(Player.Instance.playerStats, demonStats, 1);
        ...
        {
            hp -= Formulas.GetMagicalDamage(Player.Instance.playerStats, demonStats, 1);
            damageReceived.text = "" + Formulas.GetMagicalDamage(Player.Instance.playerStats, demonStats, 1);
            Vector3 diff = transform.position - target;
            if (Mathf.Abs(diff.x) > Mathf.Abs(diff.y))
            {
                if (diff.x > 0)
