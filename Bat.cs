...
public class Bat : MonoBehaviour
{
    ...
    void Attack()
    {
        ...
        Player.Instance.playerStats.hp -= Formulas.GetPhysicalDamage(batStats, Player.Instance.playerStats, 1);
    ...
    private void OnTriggerEnter2D(Collider2D coll)
    {
        ...
        {
            hp -= Formulas.GetPhysicalDamage(Player.Instance.playerStats, batStats, 1);
            damageReceived.text = "" + Formulas.GetPhysicalDamage(Player.Instance.playerStats, batStats, 1);
        ...
        {
            hp -= Formulas.GetMagicalDamage(Player.Instance.playerStats, batStats, 1);
            damageReceived.text = "" + Formulas.GetMagicalDamage(Player.Instance.playerStats, batStats, 1);
            Vector3 diff = transform.position - target;
            if (Mathf.Abs(diff.x) > Mathf.Abs(diff.y))
            {
                if (diff.x > 0)
